--[[
ReaScript name: BuyOne_Move edit cursor to previous selected note start.lua
Author: BuyOne
...
local err = notecnt == 0 and 'no notes' or cur_pos <= item_start and 'midi take start has been reached'
...
local latest_selected
	...
	local retval, sel, muted, start_pos, end_pos, chan, pitch, vel = r.MIDI_GetNote(take, i) -- only addresses notes in MIDI channels enabled in the MIDI channel filter or all notes if no channel is enabled or all are enabled
		...
		pos = r.MIDI_GetProjTimeFromPPQPos(take, pos)
			if pos >= cur_pos then break
			else -- keep storing the note whose start is the closest to the edit cursor on the left
			latest_selected = pos
			end
		end
	i=i+1
	...
	if latest_selected then
--	r.Undo_BeginBlock()
	r.SetEditCurPos(latest_selected, false, false) -- moveview, seekplay false
--	r.Undo_EndBlock('Move edit cursor to next selected note', -1)
	return r.defer(no_undo)
	end

err = sel_exist and 'no previous selected note'
