--[[
ReaScript name: BuyOne_Cycle visible Ruler lanes up or down_META.lua (2 scripts)
Author: BuyOne
...
local scr_name = fullpath:match('.+_(.+)%.%w+') -- without path, scripter name & ext // suitable for individual scripts
	...
	if not META_Spawn_Scripts(fullpath, fullpath_init, 'BuyOne_Cycle visible Ruler lanes up or down_META.lua', names_t) --  names_t is optional only if constructed outside of the function, otherwise names are collected from the list in the header
	then return r.defer(no_undo) end -- abort if META script but continue if not


--[[-------- NAME TESTING -------------
local t = {
'Cycle visible Ruler lanes up',
'Cycle visible Ruler lanes down',
}
scr_name = t[2]
--]]------------------------------------

	if not Invalid_Script_Name(scr_name, 'up', 'down')
	then return r.defer(no_undo) end

local lane_count = Get_Ruler_Lane_Count()
local t = Get_Visible_Ruler_Lanes(lane_count)

	if lane_count == 1 or #t == 1 then
	Error_Tooltip('\n\n can\'t cycle single lane \n\n', 1, 1) -- caps, spaced true
...
-- if up move top to bottom, otherwise buttom to top
-- the rest will lign up by themselves, i.e. when top is moved to bottom
-- the second moves to top, the third moves to second, etc. the bottom
-- moves to penultimate position
local a, b = t[1], t[#t]
a, b = table.unpack(up and {a,b} or {b,a})
r.GetSetProjectInfo(0, 'RULER_LANE_ORDER:'..a, b, true) -- is_set true
