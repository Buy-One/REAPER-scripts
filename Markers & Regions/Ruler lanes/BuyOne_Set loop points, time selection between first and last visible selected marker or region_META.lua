...
function Get_First_Last_Visible_Selected_Mrker_Rgn()
...
local cnt, st, fin = 0
	...
	local lane_vis = r.GetSetProjectInfo(0, 'RULER_LANE_HIDDEN:'..lane_idx, 0, false) == 0 -- isSet false // the attribute works without the colon as well // essentially redundant because B_VISIBLE is also false when the entire lane is hidden
		...
		local rgn = s ~= f -- if start and end differ it's a region // OR r.GetRegionOrMarkerInfo_Value(0, obj, 'B_ISREGION') == 1
		s, f = rgn and f or s, rgn and s or f -- if region use its end as start point and start as end point so that between two regions time selection or loop are created from end of the fist one to the start of the last one
		st = (not st or s < st) and s or st
		fin = (not fin or f > fin) and f or fin
		cnt = cnt+1
		end
	end
...
--[[-------- NAME TESTING ------------
local t = {
'Set time selection between first and last selected marker or region',
'Set loop points selection between first and last selected marker or region'
}
scr_name = t[2]
--]]----------------------------------


	if not Invalid_Script_Name(scr_name, 'set time selection', 'set loop points')
	then return r.defer(no_undo) end


local st, fin = Get_First_Last_Visible_Selected_Mrker_Rgn()
local err = not st and 'no visible selected \n\n   markers/regions'
or st == fin and '   not enough visible selected \n\n\t  markers or regions \n\n or there\'s no gap between them'
...
r.GetSet_LoopTimeRange(true, scr_name:match('loop') and true or false, st, fin, true) -- isSet true, isLoop depends on script name, true and false because the expected argument is boolean not nil or string, allowautoseek true
