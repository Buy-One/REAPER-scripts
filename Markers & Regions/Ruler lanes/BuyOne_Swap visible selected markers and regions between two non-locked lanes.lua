--[[
ReaScript name: BuyOne_Swap visible selected markers and regions between two non-locked lanes.lua
Author: BuyOne
...
function Collect_Mrkrs_Regns_From_Two_Lanes()
...
local lane1_idx, lane2_idx
	...
	local lane_vis = GetSet(0, 'RULER_LANE_HIDDEN:'..lane_idx, 0, false) == 0 -- isSet false // the attribute works without the colon as well // essentially redundant because B_VISIBLE is also false when the entire lane is hidden
		if lane_vis and not lane_locked and Get(0, obj, 'B_VISIBLE') == 1 and Get(0, obj, 'B_UISEL') == 1 then
			if lane1_idx and lane1_idx == lane_idx or lane2_idx and lane2_idx == lane_idx
			or not lane1_idx or not lane2_idx
			then
			table.insert(t, obj)
			lane2_idx = lane2_idx or lane1_idx and lane_idx ~= lane1_idx and lane_idx -- only assigning index at the 2nd loop cycle so that lane2_idx doesn't end up having the same value as lane1_idx
			lane1_idx = lane1_idx or lane_idx
			elseif lane1_idx and lane1_idx ~= lane_idx or lane2_idx and lane2_idx ~= lane_idx
			then return
			end
		end
	end

return t, lane1_idx, lane2_idx
...
local sel_obj_t, lane1_idx, lane2_idx = Collect_Mrkrs_Regns_From_Two_Lanes()
local err = not sel_obj_t and 'markers/regions are selected \n\n\ton more than two lanes'
or #sel_obj_t == 0 and '\tno visible selected \n\n\t markers/regions \n\n or their lanes are locked'
or (lane1_idx == lane2_idx or not lane2_idx) and 'markers/regions are only \n\n     selected on a single \n\n\t non-locked lane'
...
r.Undo_BeginBlock()
	...
	local src_lane_idx = r.GetRegionOrMarkerInfo_Value(0, obj, 'I_LANENUMBER')
	local dest_lane_idx = src_lane_idx == lane1_idx and lane2_idx or src_lane_idx == lane2_idx and lane1_idx
	r.SetRegionOrMarkerInfo_Value(0, obj, 'I_LANENUMBER', dest_lane_idx)
	end
