--[[
ReaScript name: BuyOne_Set toggle state depending on selected item mute state.lua
Author: BuyOne
...
About: 	A utility script only meant to be used within custom actions
			...
			state because the stock action 'Item properties: Toggle mute'
			doesn't report toggle state.
			
			The actions order must look as follows:
			
			BuyOne_Set toggle state depending on selected item mute state.lua
...
r.SetToggleCommandState(sect_ID, cmd_ID, r.GetMediaItemInfo_Value(sel_item,'B_MUTE'))
