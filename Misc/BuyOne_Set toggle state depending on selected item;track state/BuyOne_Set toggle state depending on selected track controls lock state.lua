--[[
ReaScript name: BuyOne_Set toggle state depending on selected track controls lock state.lua
Author: BuyOne
...
About: 	A utility script only meant to be used within custom actions
	...
	state because the stock action  
	'Track: Toggle lock/unlock track controls'  
	doesn't report toggle state.
	
	The actions order must look as follows:
	
	BuyOne_Set toggle state depending on selected track controls lock state.lua
...
r.SetToggleCommandState(sect_ID, cmd_ID, Track_Controls_Locked(sel_tr))
