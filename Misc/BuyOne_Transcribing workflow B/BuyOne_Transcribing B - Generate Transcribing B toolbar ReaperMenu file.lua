...
item_7=ALT
item_8= FORMAT
item_9= Prepare for rendering
item_10= Import SRT/VTT/TXT
]]

-- version to be used with builds 7.10, only 'Loop segment' is replaced with a native action
-- 'Loop to' remain linked to custom actions because they're useful with 'Real time preview' script
...
-- if no SWS and JS extensions are installed item_7 will not be populated with script command ID
...
'BuyOne_Transcribing B - Import SRT or VTT file as markers and SWS track Notes',
}
...
local toolbar2_t = {}
	for line in toolbar2:gmatch('[^\n]+') do
		if line then
		local line = line:match('item_7=') and alt_item and 'item_7= '..alt_item or line -- if valid, add name, else leave as a stub which will be ignored by REAPER on import
		toolbar2_t[#toolbar2_t+1] = line
		end
	end


-- if extensions are not installed leave field 8 blank
ref_t[8] = ext and ref_t[8] or ''


	for line in io.lines(path..'reaper-kb.ini') do -- parse action list contents
		for k, item in ipairs(ref_t) do
		local item = Esc(item)
			if (build < 7.10 and k ~= 3 -- ignoring 40634 'Clear loop points' which is 3d item and doesn't require command ID update
			or build > 7.09 and k ~= 2 and k ~= 3) -- or (k < 2 and k > 3) // in builds newer that 7.09 ignoring two items because the second 'Loop segment' is replaced with a native action 'Regions: Set loop points to current region'
			and (ext or not ext and k ~= 8) -- when extensions aren't installed item_7 (counted as 8th) is ignored and will be ignored on import by REAPER
		...
		if build < 7.10 and (ext and cntr == 10 or not ext and cntr == 9)
		or build > 7.09 and (ext and cntr == 9 or not ext and cntr == 8) then break end -- all relevant entries have been found // 8 because in later builds one custom action is replaced with the native one which is always there so doesn't require presence validation
	end


local absent = ''
	for k, item in ipairs(toolbar2_t) do
		if (build < 7.10 and k ~= 3
		or build > 7.09 and k ~= 2 and k ~= 3)
		and (ext or not ext and k ~= 8)
