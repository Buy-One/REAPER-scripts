...
-- in theory to calculate Arrange size between the bottom docker and the Ruler for horizontal zones 
-- one would have to get max track size while the bottom docker is open
-- then close all windows docked at the bottom docker in order to avoid closing the docker itself 
-- because top docker relevant for calculation will be closed as well, in case some windows are docked in it,
-- then get max track size, then re-open the closed windows docked at the bottom docker
