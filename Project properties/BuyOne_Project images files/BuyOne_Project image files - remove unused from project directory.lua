--[[
ReaScript name: BuyOne_Project image files - remove unused from project directory.lua
Author: BuyOne
...
About:  The script is meant to complement REAPER built-in
        functionality of 'Clean current project directory'
        which ignores image files.
        
        The script removes image files unused in the current
        project from the location they're stored at in the 
        project directory, which is either project media path
        or a dedicated folder if specified in the IMAGE_DIRECTORY 
        setting. 
        
        The script accounts for item notes image files, video 
        take source image files and track icon files. Image 
        files located in the /Data folder in REAPER resource 
...
-- to be used inside the project directory;
-- if the setting is empty, project media directory will be used,
-- if invalid or not found a prompt will allow to choose further
-- course of actions
IMAGE_DIRECTORY = ""

-- Between double square brackets provide full path to the directory
-- where you'd like the removed files to be sent
-- for temporary storage if you don't want them
-- to be deleted from disk,
-- REAPER provides an option to send files to the OS recycle bin
-- but doing so using Lua isn't a trivial task,
-- hence this setting;
-- IF THE FILE PATH IN THIS DIRECTORY WILL EXCEED THE PATH
-- LENGTH LIMIT OF 256 CHARACTERS, BEFORE BEING MOVED THE FILE
-- NAME WILL BE TRUNCATED FOR THE PATH TO MEET THE LIMIT,
-- HOWEVER IF THIS DOESN'T HELP, THE FILE WON'T BE MOVED
-- REMANING IN ITS ORIGINAL LOCATION
TRASH_DIRECTORY = [[ ]]
