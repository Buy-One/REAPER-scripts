...
r.Main_OnCommand(cmd_ID,0)
	...
	local tr = r.GetSelectedTrack(0,i)
		...
		local retval = r.CreateTrackSend(tr, dest_tr)
			...
			r.SetTrackSendInfo_Value(tr, 0, k-1, 'I_SENDMODE', SEND_MODE) -- category 0 (send), sendidx k-1 because send indices count is 0-based // since the track is new it doesn't have sends therefore table keys can be used for numbering, with existing tracks r.GetTrackNumSends(tr, 0)-1 must be used
