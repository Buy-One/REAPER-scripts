...
DESTINATION_TRACK_NAME = DESTINATION_TRACK_NAME:match('^%s*(.-)%s*$')--('[%w%p].*[%w%p]*')
...
local bus_tr
	...
	local retval, name = r.GetTrackName(tr)
		if name:match('^%s*'..Esc(DESTINATION_TRACK_NAME)..'%s*$') then bus_tr = tr break end
	end
...
SEND_MODE = SEND_MODE < 3 and SEND_MODE-1 or SEND_MODE -- to conform to 0-based index of post-fader (0) and pre-fx (1)
