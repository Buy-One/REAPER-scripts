...
About: 	The script is an emulation of a Pro Tools feature
		...
		while the mouse cursor hovers over a razor edit area. Once 
...
-- which the script relies on, so having not detected such data it deselects tracks
	...
	local exist = #exist > 0
	local ret, RE_track = r.GetSetMediaTrackInfo_String(tr, 'P_EXT:LINK_TRACK_AND_EDIT_SELECTION', '', false) -- setNewValue false
	RE_track = #RE_track > 0
