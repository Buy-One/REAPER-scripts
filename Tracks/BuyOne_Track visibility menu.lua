...
Version: 1.1
Changelog: 1.1 #Fixed logic of detection of the first visible track in the Mixer
Licence: WTFPL
...
local st, fin = t and 1 or 0, t and #t or r.CountTracks(0)-1
	...
	local tr = t and t[i] or r.GetTrack(0,i)
		if (not mixer_vis and r.IsTrackVisible(tr, mixer_vis) or mixer_vis and r.CSurf_TrackToID(tr, mixer_vis) ~= -1) -- CSurf_TrackToID() ensures that the MCP is both not hidden and not inside a collapsed folder, because if an MCP is in a collapased folder its original I_MCPX value is maintained, simple visibility evaluation with r.IsTrackVisible(tr, true) will result in false positive and a track inside a collapsed folder which precedes the actual first visible MCP will be resurned instead; for tracks in Arrange IsTrackVisible() is enough because unlike for the Mixer it returns false when tracks inside a collapsed folder are hidden as per the preference at Preferences -> Appearance -> Track Control Panels -> Folder collapse button cycles track heights
